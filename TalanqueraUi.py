    ...
    def actualizar(self, dbruote, modo):
        ...
        try:
            ...
            with CallServer(WSURL) as ws:
                ...
                log.write("# [func.actualizar]:Inicio de bucle...\n")
                for codTarjeta, bloque in response.iteritems():
                    ...
                    if tipo == '2':
                        ...
                        SQLsUpdate.append(sql)
                        counterI += 1
                # log.write("# [func.actualizar]:Se construye un 'in clause' "
                #          "para determinar cantidad real de updates. Valor del in: {0}\n".format(in_clause))
                log.write("# [func.actualizar]:El listado de sqls a ejecutar:  {0}\n".format(SQLsUpdate))
            ...
            if modo == 'odbc':
                ...
                # log.write("# [func.actualizar]:Sql para verificar cantidad más exacta "
                #          "de updates: verifier={0}\n".format(verifier))

                log.write("# [func.actualizar]:Se ejecutaran los sql enlistados. Sentencias:\n")
                for sentencia in SQLsUpdate:
                    cur.execute(sentencia)
                    log.write("# [func.actualizar]:{0}\n".format(sentencia))

                log.write("# [func.actualizar]:Final de sentencias enlistadas...\n")
                # cur.execute(verifier)
                # log.write("# [func.actualizar]:Se ha ejecutado el sql para contar los updates...\n")
                # t = tuple(cur)
                # log.write("# [func.actualizar]:Resultado del sql ejecutado:  {0}\n".format(t))
                # counterU = len(t)
